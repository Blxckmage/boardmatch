...
3. `Read` to read, `Edit` to edit, `Write` for new files only. No new docs/abstractions/deps unless asked.
4. Linear is the todo list: `Boardmatch` project, team `BLA` — pull its Backlog before starting, work bottom-up (Urgent foundation first), set In Progress on start and Done + comment on finish. Stack lives in `.docs/ARCHITECTURE.md`, never mirrored here.
